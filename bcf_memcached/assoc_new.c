...
static void *assoc_maintenance_thread(void *arg) {
    ...
    while (do_run_maintenance_thread) {
        ...
        if (!expanding) {//不再需要迁移数据了。
            ...
            //挂起扩展线程，直到别的线程插入数据后发现item数量已经
            //到了1.5倍哈希表大小,此时调用别的线程调用assoc_start_expand
            //函数，该函数会调用pthread_cond_signal ,唤醒扩展线程 
           ...
           //从maintenance_cond条件变量中醒来，说明又要开始扩展哈希表
           //和迁移数据了。迁移线程在迁移一个桶的数据时是锁上全局级的锁.  
