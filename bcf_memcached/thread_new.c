...
//----------------------------->>>>.>>
//    因为所有的workers线程都在处于event_base循环中，
//可以直接往workers线程监听的管道中写入一个字节就能通知workers线程了。
//    迁移线程为什么要这么迂回曲折地切换workers线程的锁类型呢？直接修改所有线程的LIBEVENT_THREAD结构的item_lock_type成员变量不就行了吗？
//    这主要是因为迁移线程不知道worker线程此刻在干些什么。
//如果worker线程正在访问item，并抢占了段级别锁。此时你把worker线程
//的锁切换到全局锁，等worker线程解锁的时候就会解全局锁(参考前面
//的item_lock和item_unlock代码)，这样程序就崩溃了。所以不能迁移
//线程去切换，只能迁移线程通知worker线程，然后worker线程自己去切换。
//当然是要worker线程忙完了手头上的事情后，才会去修改切换的。
//所以迁移线程在通知完所有的worker线程后，会调用wait_for_thread_registration
//函数休眠等待所有的worker线程都切换到指定的锁类型后才醒来。
//<<.<<<<<-------------------------------
...
static void *worker_libevent(void *arg) {//这个函数也是在初始化时调用的
    ...
    //为workers线程设置线程私有数据
    //因为所有的workers线程都会调用这个函数，
    //所以所有的workers线程都设置了相同键值的,线程私有数据
