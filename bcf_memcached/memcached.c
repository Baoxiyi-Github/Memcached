...
static int max_fds;
static struct event_base *main_base;//主线程的event_base
...
static void settings_init(void) {
    ...
    settings.inter = NULL;
    settings.maxbytes = 64 * 1024 * 1024; /* default is 64MB *///memcached能够使用的最大内存
    //最多允许多少个客户端同时在线。不同于settings.backlog 
    ...
    settings.item_size_max = 1024 * 1024; /* The famous 1MB upper limit. *///slab内存页的大小。单位是字节
    //如果连接数超过了最大同时在线数(由-c选项指定)，是否立即关闭新连接上的客户端
...
//注意，主线程对于socket fd 可读事件的回调函数是event_handler，
//回调参数是conn这个结构体指针
...
static int server_socket(const char *interface,
    ...
    //一个主机可能会有多个网卡，比如双线机房，联通或者电信
    //如果interface是一个hostname的话，那么可能就有多个ip
    for (next= ai; next; next= next->ai_next) {
        conn *listen_conn_add;

        //创建一个套接字，然后设置为非阻塞的
        if ((sfd = new_socket(next)) == -1) {//调用socket函数
...
static int server_sockets(int port, enum network_transport transport,
    ...
    } else {//settings.inter里面可能有多个IP地址.如果有多个那么将用逗号分隔
...
/*
 *------------------------------>>>>>
 *worker线程对于管道可读事件的回调函数是thread_libevent_process函数。
...
int main (int argc, char **argv) {
    ...
    //------启动数据迁移线程------
    //  迁移线程被创建后会进入休眠状态(通过等待条件变量)，
    //  当worker线程插入item后，发现需要扩展哈希表就会调用assoc_start_expand函数唤醒这个迁移线程
    ...
    if (settings.socketpath == NULL) {
        ...
        //开始通过端口号启动网络监听事件,创建监听客户端的socket
