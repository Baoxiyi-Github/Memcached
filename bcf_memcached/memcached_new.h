...
//可以看到每个线程的线程私有数据是每个线程
//都独有的LIBEVENT_THREAD结构体的成员变量item_lock_type。
//只要根据需要把workers线程的item_lock_type变量修改就
//可以完成锁的切换。哈希表迁移线程会在assoc.c文件中
//的assoc_maintenance_thread函数调用switch_item_lock_type函数，
//让所有的workers线程都切换到段级别锁或者全局级别锁
