...
typedef struct {
    ...
    unsigned int size; //slab分配器分配的item的大小 /* sizes of items */
    unsigned int perslab;//每一个slab分配器能分配多少个item /* how many items per slab */

    void *slots;//指向空闲item链表  /* list of item ptrs */
    unsigned int sl_curr;//空闲item的个数/* total free items in list */

    //这个是已经分配了内存的slabs个数。list_size是这个slabs数组(slab_list)的大小
    unsigned int slabs;//本slabclass_t可用的slab分配器个数 /* how many slabs were allocated for this class */
...
static void *slab_maintenance_thread(void *arg) {
    ...
    while (do_run_slab_thread) {
        if (settings.slab_automove == 1) {//启动了automove功能
            ...
            //调用slab_automove_decision判断是否应该进行内存页重分配。
            //返回1就说明需要重分配内存页，此时调用slabs_reassign进行处理。
