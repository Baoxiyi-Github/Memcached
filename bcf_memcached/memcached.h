...
struct settings {
    ...
    int tail_repair_time; //存储时间距离  /* LRU tail refcount leak repair time */
    bool flush_enabled;     /* flush_all enabled */
    char *hash_algorithm;     /* Hash algorithm in use */
    int lru_crawler_sleep; //默认休眠时间为100微秒 /* Microsecond sleep between items */

    //指明最多只检查每一条LRU队列的多少个item。默认值为0
    //所以如果不指定那么就不会工作
...
typedef struct _stritem {
    ...
    struct _stritem *h_next;//h_next指针用于哈希表的冲突链/* hash chain next */
    rel_time_t      time;//最后一次访问时间。绝对时间/* least recent access */
    rel_time_t      exptime;//过期失效时间，绝对时间/* expire time */
    int             nbytes;//本item存放的数据的长度     /* size of data */
    ...
    //item结构体的最后一个成员是data[]，这样的定义称为柔性数组
    //性数组的一个使用特点是，数据域就存放在数组的后面。
    //memcached的item也是这样使用柔性数组的。
    //----上面的item只是定义了item结构体本身的成员，
    //但之前的博文一直用item表示item存储的数据。
    //这样写是合理的。因为item结构体本身和item对应的数据
    //都是存放slab分配器分配的同一块内存里面-----
    union {
        uint64_t cas;
        char end;
    } data[];
...
struct conn {
    ...
    char   *rbuf; //缓存数据的起始地址，存放的始commands信息  /** buffer to read commands into */
    char   *rcurr;//当前所在的缓存偏移量 /** but if we parsed some already, this is where we stopped */
    int    rsize;//缓存数据的大小，也就是c->rbuf的大小，通过malloc()函数创建   /** total allocated size of rbuf */
    int    rbytes;//从c->rcurr开始还有多少未读数据  /** how much data, starting from rcur, do we have unparsed */
